...
labDat <- data.table(read.csv("labs.csv",head=T,stringsAsFactors=F))[DRUG=="MOXIFLOXACIN",c("VAR","LAB"),with=F]
...
simPar <- function(indDat,input){
  with(c(indDat,input,par), {
    ...
    # Clearance
    CLi = (CL*((1/100)*pCL))*exp(OM1*sqrt(ETA1))*(WT/70)**(0.75)
    # Central volume of distribution
    V2i = (V2*((1/100)*pV2))*exp(OM2*sqrt(ETA2))*(WT/70)**(1)
    # Peripheral volume of distribution
    V3i = (V2*((1/100)*pV3))*(WT/70)**(1)
    ...
    TMIC <- function() {
      root <- uniroot.all(function(x)(nest(x))-0.5,c(0,II))
    ...
    # (TMIC())
    list(CL=CLi,V2=V2i,V3=V3i,D1=pD1,Q=Qi,A1=A1,AUC=A1/CLi,CMAX=R*(C1/l1*(1-exp(-l1*pD1))+C2/l2*(1-exp(-l2*pD1))),TMAX=pD1,CSS=(A1/CLi)/II,TMIC=(TMIC()/II)*100, MICAUC=(A1/CLi)/(II/24),TOTAL=0)
  })
}

# par <- list(CL=14.6,V2=10.8,V3=12.6,D1=0.5,Q=18.6,ETA1=0.118,ETA2=0.143,ETA3=0.290,ETA4=0.102,EPS=0.035, pV3=100, pQ=100)

par <- list(CL=12.1,V2=68.1,V3=44.6,D1=1,Q=20.3,ETA1=0.0633,ETA2=0.0725, EPS1=0.00465, EPS2=0.034, pV3=100, pQ=100, pCL=100, pV2=100)
...
shinyServer(function(input, output) {
   ...
   output$corPlot <- renderPlot({
    ...
    pCur <- switch(input$STRAT,
                 SEX = ggplot(simPop(),aes_string(X,Y,col='c("Female","Male")[factor(SEX)]'))+layer1+labs(y=labY,x=labX,title="Moxifloxacin"),
                 AGE = ggplot(simPop(),aes_string(X,Y,col='c("<45","45+")[1+(AGE>45)]'))+layer1+labs(y=labY,x=labX,title="Moxifloxacin"),
    ...
    if (Y=="MICAUC") pCur <- pCur + geom_hline(yintercept=30*2,linetype=2,col="purple",alpha=1/2)+ geom_hline(yintercept=200,linetype=2,col="purple",alpha=1/2)
    

     if(input$lm) pCur <- pCur + geom_smooth(method="lm",se=FALSE)
    
     pCur+theme_bw()+labs(x=labX,y=labY,col="",title="Moxifloxacin")
   })

...
shinyUI(navbarPage(
  ...
  titlePanel("Rivaroxaban"),
  
  tabPanel("Plot",
           sidebarLayout(
             sidebarPanel(
               sliderInput("DOSE1", "Dose (mg)",  min = 1, max = 200, step = 1, value = 20),
               sliderInput("DOSE2", "Dose in HIV patients (mg)",  min = 1, max = 200, step = 1, value = 20),
  ...
  tabPanel("Correlations",
           sidebarLayout(
             sidebarPanel(
               ...
               selectInput("COR2","X-axis:",choices=c("Clearance"="CL","Volume of Distribution"="V2","Area Under the Curve"="AUC","Maximum Concentration"="CMAX","Time of maximum Concentration"="TMAX","Weight"="WT","Height"="HT","Sex"="SEX","Age"="AGE","Serum Creatinine"="SCR","Albumin"="ALB","Lean Body Mass"="LBM","Body Mass Index"="BMI","Creatinine Clearance"="CRCL","BSA"="BSA","Ritonavir"="RTV","Total dataset"="TOTAL"),selected="WT"),
               checkboxInput("lm","Linear regression"),
               selectInput("STRAT","Stratify by:",choices=c("None"=0,Sex="SEX",Age="AGE",Ritonavir="RTV")),
               submitButton("Update Graph", icon=icon("refresh", class="fa-spin"))
             ),
             mainPanel(
               plotOutput("corPlot"),
